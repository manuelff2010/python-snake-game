...
class game_logic():
            ...
            if not(verificacion_cordenada[0] == None and verificacion_cordenada[1] == None):
                if verificacion_cordenada[0] == None:
                    if verificacion_cordenada[1]:
                        self.nueva_cordenada[1] = 0
                    else:
                        self.nueva_cordenada[1] = self.limits
                else:
                    if verificacion_cordenada[0]:
                        self.nueva_cordenada[0] = 0
                    else:
                        self.nueva_cordenada[0] = self.limits
            self.cola.insert(0,self.nueva_cordenada)
        if len(self.cola) > self.tamaño_serpiente + 2:
            self.cola.pop(-1)
    def validacion_manzana(self):
        if self.manzana != None:
            if self.cola.count(self.manzana) > 0:
                self.manzana = None
    ...
    def update(self):
        self.movimiento()
        if self.validacion_muerte() == 1:
            return 1
        self.validacion_manzana()
